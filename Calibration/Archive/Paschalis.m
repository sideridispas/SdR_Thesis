...
%Load data
PCB = load('./before/PCB_data_before.csv');
DMM = load('./before/DMM_data_before.csv');
...
saveas(gcf, 'Means', 'png')

% calc the difference
difference  = [PCBmean-DMMmean];
% plot to do some regresion analyses
figure 
plot(DMMmean(1:end-2),difference(1:end-2),'o')
ylabel('difference')
xlabel('DMM reading')
